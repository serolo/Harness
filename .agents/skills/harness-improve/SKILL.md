...
description: >
  ...
  AGENTS.md files, .Codex/rules/*, ci/harness-gates.sh, or scaffolds new skills. Do NOT
...
guides and sensors that shapes AI-assisted work in this repo (AGENTS.md files, `.Codex/rules/*`,
...
> Never edit `AGENTS.md`, `.Codex/rules/*`, or `ci/harness-gates.sh` until the developer has
...
| Layer | File(s) | Role |
|---|---|---|
| Rules-as-code | `.Codex/rules/{security,architecture,conventions}.md` | Tagged `[GATE]`/`[REVIEW]`; guide AI review + compile to gates |
| Always-on context (local) | any nested `AGENTS.md` (add one when a subsystem's logic is non-obvious) | Per-subsystem behaviour, gotchas, integrations |
| Deterministic gates | `ci/harness-gates.sh` | gates: `format` / `lint` / `typecheck` / `check` (no-arg = `npm run check`); blocking |
| Specialists | `.Codex/agents/*.md` | spec-writer, coder, test-author, code-review, verifier, deep-dive, … |
| Progressive skills | `.Codex/skills/<name>/SKILL.md` | Task-triggered workflows (`/harness-plan`, `/harness-implement`, `/harness-review`) |
| Hooks | `.Codex/settings.json` (+ any `.Codex/hooks/*`) | Pre/Post/Stop enforcement |
...
|---|---|---|
| A language/library footgun (TS/JS, async, Electron/IPC, logging) that applies **repo-wide** | `.Codex/rules/conventions.md` (or a root `AGENTS.md` if you create one) | A tagged rule — add a **BAD / GOOD code pair** when a concrete snippet makes it clearer |
| Behaviour specific to **one subsystem** | the **nearest** nested `AGENTS.md` (create one under that subsystem if absent) | Purpose / How it works / Gotchas — practical, not exhaustive |
| A **security / architecture / convention standard** | the matching `.Codex/rules/*.md` | **one tagged line** that *points at* the detail; don't restate detail that already lives in a `AGENTS.md` or `code-review.md` |
...
| A **repeatable multi-step task pattern** (recurring work-shape) | a **new skill** under `.Codex/skills/<name>/SKILL.md` | narrow, anti-trigger-rich `description`; `disable-model-invocation: true` for explicit `/`-only skills |
...
1. **Promotion candidates** — `[REVIEW]` rules in `.Codex/rules/*` and `AGENTS.md` that a linter /
   formatter / script could now enforce → recommend `[REVIEW]` → `[GATE]` + the gate wiring.
2. **Rule-noise** — duplicate, overlapping, or contradictory rules across root `AGENTS.md` and the
...
5. **Doc gaps** — subsystems with non-obvious logic and no nearest `AGENTS.md` (there are none in
...
- If a root-`AGENTS.md` behaviour changed, root `AGENTS.md` is updated (it's the contract).
