...
> means didn't write the code). Both must obey the house rules in the root `AGENTS.md` and
> `.Codex/rules/` — restate that in every agent prompt. You run in the **main session**: only you can
...
any nested `AGENTS.md` / `.Codex/rules/` referenced in its "Read before implementing" section.
...
**Handoff:** run `/verify` (evidence), then `/harness-review` (or comment `/Codex-review` on the PR).
