...
describe('MODEL_PATTERN', () => {
  it('accepts a provider-qualified model as one inert argument', () => {
    ...
    expect(MODEL_PATTERN.test('opus[1m]')).toBe(true);
    expect(MODEL_PATTERN.test('anthropic/opus[1m]')).toBe(true);
  });
  ...
  it('rejects paths and shell syntax', () => {
    ...
    expect(MODEL_PATTERN.test('opus[2m]')).toBe(false);
    expect(MODEL_PATTERN.test('opus[1m];rm')).toBe(false);
  });
});
