...
### 2. Read the codebase
Load `.Codex/rules/{security,architecture,conventions}.md` (and the nearest `AGENTS.md`, if the
...
- Process-boundary traps: does this cross the main↔renderer line? The renderer stays sandboxed
  ...
  `src/shared/*` types. See `.Codex/rules/architecture.md`.
- **Heightened-scrutiny paths** (`.Codex/rules/security.md`): the IPC/preload boundary,
