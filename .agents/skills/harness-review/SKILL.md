---
name: harness-review
description: PIV step 4 (local). Delegate the working-tree diff to the code-review agent, judge it against .Codex/rules and the ticket's acceptance criteria, and write reports/<slug>-review.md. Invoke explicitly with /harness-review. For the PR-time AI review, comment /Codex-review instead.
...
> **Don't duplicate the PR review.** On a PR, the canonical AI review is the **`/Codex-review`**
> comment (→ `pr-Codex-review.yml` → the `code-review` agent). Use `/harness-review` for a *local*
...
2. **Delegate to the `code-review` agent.** It runs lint/types, judges quality against
   `.Codex/rules/{security,architecture,conventions}.md`, validates the ticket's acceptance
   ...
   secrets/tokens, packaging — see `.Codex/rules/security.md`), and ends with a PASS/FAIL verdict.
...
`/Codex-review` for the gated AI pass + a human architecture review (two-reviewer minimum).
