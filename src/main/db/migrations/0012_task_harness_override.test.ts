...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'harness-task-harness-migration-'));
...
function taskColumns(): string[] {
  return (
    db.pragma('table_info(scheduled_tasks)') as Array<{ name: string }>
  ).map((column) => column.name);
}

describe('migration 0012 task harness override', () => {
  ...
  it('upgrades v11 without changing existing task data', () => {
    runMigrations(db);
    db.exec('ALTER TABLE scheduled_tasks DROP COLUMN harness_override;');
    ...
    db.prepare(
      `INSERT INTO scheduled_tasks
        (id, workspace_id, prompt, model, mode, scheduled_at, state, origin,
         turn_id, error_message, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    ).run(
      'task-1',
      'workspace-1',
    ...
    expect(
      db
        .prepare(
          'SELECT prompt, model, harness_override FROM scheduled_tasks WHERE id = ?',
        )
        .get('task-1'),
    ).toEqual({
      prompt: 'preserve me',
