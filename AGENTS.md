# AGENTS.md — Harness (root working agreement)
...
> `.Codex/rules/{security,architecture,conventions}.md` (tagged `[GATE]`/`[REVIEW]`). Per-subsystem
> notes live in the nearest nested `AGENTS.md` (e.g. `src/main/ipc`, `src/main/pty`, `src/main/git`).
...
`AGENTS.md`. Heightened-scrutiny paths (`.Codex/rules/security.md`) get a named review.
